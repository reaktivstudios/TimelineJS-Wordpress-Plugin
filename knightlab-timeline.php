...
function kl_timeline_shortcode( $atts, $content = null ) {
	...
	if ( 'timeline3' === $atts['version'] ) {
		$atts['script_path'] .= 'v3/js/';
		wp_register_script( 'kl-timeline-embed', $atts['script_path'] . 'timeline-embed.js', array( 'jquery' ), false, true );
	} else {
		$atts['script_path'] .= 'js/';
		wp_register_script( 'kl-timeline-embed', $atts['script_path'] . 'storyjs-embed.js', array( 'jquery' ), false, true );
	}

	if ( $atts['debug'] ) {
		$js_path = $atts['script_path'] . 'timeline.js';
	} else {
		$js_path = $atts['script_path'] . 'timeline-min.js';
	}
	...
	$shortcode = '
		...
		<script type="text/javascript">// <![CDATA[
			var timeline_config = {
				...
				start_at_end: ' . $atts['start_at_end'] . ',
				start_at_slide: "' . $atts['start_at_slide'] . '",
				start_zoom_adjust: "' . $atts['start_zoom_adjust'] . '",
...
<script type="text/javascript">
		...
		<div class="wrap">
			<div>
				<div style="padding:15px 15px 0 15px;">
					...
					<span>
						<?php esc_html_e( 'Configurate your Timeline and add it to your post', 'kl-timeline' ); ?>
					</span>
				</div>
				<div style="padding:15px 15px 0 15px;">
					<table>
	...
	<br/>
				...
				<div style="padding:15px;">
					<input type="button" class="button-primary" value="<?php esc_html_e( 'Insert Timeline', 'kl-timeline' ); ?>" onclick="insertTimeline();"/>
